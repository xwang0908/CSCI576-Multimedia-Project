...
## 3. Priority for the Next Round

Ordered by accuracy gain per hour of work:

| # | Owner | Change | Expected effect |
|---|---|---|---|
| 1 | C | Filter <3 s non_content segments | Precision on test_002 climbs from 21 % → ~50 %, segment count 168 → ~50 |
| 2 | B | Tighten silence threshold + min duration | Removes 20+ false silence segments |
| 3 | C | Drop / narrow position-based intro rule | Removes mid-video false intros |
| 4 | B | Add `detected_keywords` per spec | Lets C's existing sponsor/outro rules fire |
| 5 | A | Average CLIP across all sampled frames | Reduces label noise |
| 6 | A | Re-balance `visual_type` decision (restore `talking_head`) | Helps C distinguish lecture from ad |
| 7 | A | Output cut-density and `has_face` features | Richer features for C to break ties |
| 8 | B | Stronger LLM prompting for sponsor detection | Improves source signal |
...
```bash
# Run pipeline (test_001 and test_002)
for n in 001 002; do
...
python backend/evaluate.py --all
```

Inputs: `test/videos/test_*.mp4` (not in git, get from team drive).
Ground truth: `test/ground_truth/test_*.json` (in repo).
Outputs: `output/test_*/segments.json`.
