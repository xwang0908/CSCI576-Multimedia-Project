...
VIDEO_FILENAME = "test_002.mp4"
AUDIO_FILENAME = "temp_audio.wav"

FINAL_OUTPUT_JSON = "final_video_analysis.json"

DEBUG_BLOCK_INPUT_TXT = "debug-block-input.txt"
DEBUG_BLOCK_OUTPUT_JSON = "debug-block-output.json"
...
def build_block_classification_prompt(blocks, total_duration, global_summary):
    block_texts = []
...
"""
    ...
    with open(DEBUG_BLOCK_INPUT_TXT, "w", encoding="utf-8") as f:
        f.write(prompt)
...
def classify_blocks_with_llm(blocks, total_duration, global_summary):
...
"""
    ...
    try:
        ...
        with open(DEBUG_BLOCK_OUTPUT_JSON, "w", encoding="utf-8") as f:
...
def main():
    ...
    print(f"🧾 Debug block 输入保存至: {DEBUG_BLOCK_INPUT_TXT}")
    print(f"🧾 Debug block 输出保存至: {DEBUG_BLOCK_OUTPUT_JSON}")
